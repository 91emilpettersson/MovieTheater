...
The two core endpoints *api/theater/seats* and *api/theater/book* are accessed through the [TheaterController](). A mediator pattern, using the MediatR-nuget, sends the requests to the [TheaterService]() which is responsible for the domain logic (aka business logic). This in turn accesses and saves data in the [TheaterDataService](), which takes the place of a database. The TheaterDataService is a mock of a database and is not much more than a dictionary that is kept alive through the lifetime of the application. Dependency Injection is setup using the Autofac-nuget.
...
The frontend consists of three main React components; [SeatGrid](), [SeatButton]() and [BookButton]().
...
#### Unit tests
[Unit tests]() are added for the TheaterService to cover the domain logic. TheaterDataService is mocked using the Moq-nuget when constructing TheaterService in the test class.

#### Integration tests
There are two [integration tests](), one that covers only a happy case and one that tests concurrent booking.
...
in command line. This will also save a video of the test in [test-results](). This video is very short but can be stepped through to show the steps that are happening.

## Setting up and running the application.

### Backend
Open [MovieTheaterBackend.sln]() from Visual Studio and run it. Take note of which URL your application is using (e.g. https://localhost:5000).

### Frontend
- Set the URL above as the BASE_URL in the config.json file.
- Install yarn. 
- Go to the [frontend]() folder and run ´´´yarn install´´´ in command line.
